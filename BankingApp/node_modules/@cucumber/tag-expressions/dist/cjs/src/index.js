...
function parse(infix) {
    ...
    function pushExpr(token, stack) {
        if (token === 'and') {
            var rightAndExpr = popOperand(stack);
            stack.push(new And(popOperand(stack), rightAndExpr));
        }
        else if (token === 'or') {
            var rightOrExpr = popOperand(stack);
    ...
    function popOperand(stack) {
        if (stack.length === 0) {
            throw new Error("Tag expression \"".concat(infix, "\" could not be parsed because of syntax error: Expected operand."));
        }
        return stack.pop();
    }
}
